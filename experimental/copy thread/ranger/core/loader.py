...
class CopyLoader(Loadable, FileManagerAware):
    ...
    def __init__(self, copy_buffer, do_cut=False, overwrite=False):
        ...
        self.copyThread = shutil_g.CopyThread(None, None)
            ...
            if self.do_cut:
                ...
                for f in self.copy_buffer:
                    for _ in shutil_g.move(src=f.path,
                            dst=self.original_path, copyThread=self.copyThread,
                            overwrite=self.overwrite):
                        self.percent = float(self.copyThread.done) / size * 100.
                        yield
            else:
                if len(self.copy_buffer) == 1:
    ...
    def pause(self):
        Loadable.pause(self)
        self.copyThread.pause = True

    def unpause(self):
        Loadable.unpause(self)
        self.copyThread.pause = False
