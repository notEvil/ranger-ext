...
class CopyThread(object):
    def __init__(self, src, dst, length=BLOCK_SIZE):
        self.src = src
        self.dst = dst
        self.length = length
...
def copyfile(src, dst, copyThread):
    ...
    copyThread.start()
    while True:
        if not copyThread.isAlive():
            break
        time.sleep(0.033)
        yield

    if copyThread.exception != None:
        raise copyThread.exception
...
def copy2(src, dst, copyThread, overwrite=False, symlinks=False):
    ...
    if symlinks and os.path.islink(src):
        ...
        for _ in copyfile(src, dst, copyThread):
            yield
        copystat(src, dst)
...
def copytree(src, dst, copyThread, symlinks=False, ignore=None, overwrite=False):
    ...
    for name in names:
        ...
        try:
            if symlinks and os.path.islink(srcname):
                ...
                for _ in copytree(srcname, dstname, copyThread, symlinks,
                        ignore, overwrite):
                    yield
            else:
                # Will raise a SpecialFileError for unsupported file types
                for _ in copy2(srcname, dstname, copyThread,
...
def move(src, dst, copyThread, overwrite=False):
    ...
    try:
        ...
        if os.path.isdir(src):
            ...
            for _ in copytree(src, real_dst, copyThread, symlinks=True, overwrite=overwrite):
                yield
            rmtree(src)
        else:
            for _ in copy2(src, real_dst, copyThread, symlinks=True, overwrite=overwrite):
                yield
            os.unlink(src)
